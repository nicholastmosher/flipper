...
pub struct AtsamDevice<'a, T: LfDevice> {
    modules: Modules,
    uart: Uart0<'a, T>,
}

impl<'a, T: LfDevice> Read for AtsamDevice<'a, T> {
...
impl<'a, T: LfDevice> Write for AtsamDevice<'a, T> {
...
impl<'a, T: LfDevice> LfDevice for AtsamDevice<'a, T> {
    fn modules(&mut self) -> &mut Modules {
        &mut self.modules
    }
}

impl<'a, T: LfDevice> AtsamDevice<'a, T> {
